...
class cube():
    ...
    def _clean_data(self,
        ...
        if (fits_header['EXPTIME'] < 0.1):
            ...
            dummy = arrays.copy()
            arrays = np.zeros((int(dummy.shape[0]/10), dummy.shape[1], dummy.shape[2]))
            for i in range(0, int(dummy.shape[0]/10)):
                arrays[i, :, :] = np.sum(dummy[i*10:i*10+10, :, :], axis=0)
            fits_header['EXPTIME'] = fits_header['EXPTIME']*10.
            print('Data cube was reshaped to shape '+str(arrays.shape))
            print('Lost '+str(dummy.shape[0] % 10)+' files due to stacking')
            
#            if (arrays.shape[0] % 10 == 0):
#                dummy = arrays.copy()
#                arrays = np.zeros((int(dummy.shape[0]/10), dummy.shape[1], dummy.shape[2]))
#                for i in range(0, int(dummy.shape[0]/10)):
#                    arrays[i, :, :] = np.sum(dummy[i*10:i*10+10, :, :], axis=0)
...
#                print('Data cube was reshaped to shape '+str(arrays.shape))
#            else:
#                raise UserWarning('Number of frames not divisible by 10')
        ...
        flat = pyfits.getdata(self.rdir+flat_path, 0)*ratio
        bad_pixels = pyfits.getdata(self.rdir+flat_path, 1)
        
        # Set size of sub-arrays
        if (fits_header['NAXIS1'] < 256+128):
            sub_size = 64 # USER
        else:
            sub_size = 128 # USER
        file = open('log.txt', 'a')
        file.write('Data cube shape = '+str(arrays.shape)+', sub-array size = '+str(sub_size)+'\n')
        file.close()
        ...
        maxima = np.zeros(arrays.shape[0])
        ...
        for i in range(arrays.shape[0]):
            ...
            saturated_pixels = self._saturated_pixels(frame)
            bad_pixels[saturated_pixels] = 1
...
#            if (make_plots == True):
        ...
        # Check if broken quadrant is on sub-arrays
        if (np.sum(sub_stripes_map) > 0.5):
            print('Broken quadrant on sub-arrays')
            if (sub_size > 64+32):
                print('Reduced size of sub-arrays')
                sub_size = 64
                sub_stripes_map = np.roll(np.roll(stripes_map,\
                                                  int(sub_size/2.)-max_y, axis=0),\
                                                  int(sub_size/2.)-max_x, axis=1)[0:sub_size, 0:sub_size]
                ...
                if (np.sum(sub_stripes_map) > 0.5):
                    ...
                    # Skip data cube
                    file = open('log.txt', 'a')
                    file.write('Broken quadrant on sub-arrays, skipping data cube\n')
                    file.close()
                    return None
        ...
        out_name = str(fits_header['HIERARCH ESO OBS TARG NAME'])+'_'+\
...
#        col2 = pyfits.Column(name='ypeak', format='E', array=max_y)
        col3 = pyfits.Column(name='pa', format='E', array=parangs)
